...
"""
import os, sys, csv, json, datetime, urllib.parse, urllib.request, urllib.error
...
def youtube_uploads():
    ...
    try:
        ...
        out, token = [], None
        while True:
            params = {"part":"contentDetails,snippet","playlistId":playlist,"maxResults":50,"key":key}
...
def tiktok_access_token():
    """Refresh-token → access token. Persists TikTok's rotated refresh token."""
    key, secret = os.environ.get("TIKTOK_CLIENT_KEY"), os.environ.get("TIKTOK_CLIENT_SECRET")
    if not (key and secret): return None
    refresh = None
    try: refresh = json.load(open(TIKTOK_TOKEN_CACHE)).get("refresh_token")
    except Exception: refresh = os.environ.get("TIKTOK_REFRESH_TOKEN")
    ...
    try:
        ...
        if d.get("refresh_token"):
            try: json.dump({"refresh_token": d["refresh_token"]}, open(TIKTOK_TOKEN_CACHE, "w"))
            except Exception: pass
...
def main():
    ...
    # ---- Safety guard: never wipe a good dashboard with an empty pull ----------
    # If this run fetched nothing (all four platforms empty, no downloads, no daily
    # history) — almost always because API keys are missing — refuse to overwrite an
    # existing non-empty data.js. Wiping the "current state of the world" to zeros
    # because .env wasn't loaded is worse than doing nothing.
    ...
    if fetched_nothing and os.path.exists(DATA_JS_PATH):
        try:
            import re as _re
            prev = _re.search(r"window\.DASH\s*=\s*(\{.*\});", open(DATA_JS_PATH, encoding="utf-8").read(), _re.S)
            prev_data = json.loads(prev.group(1)) if prev else {}
            had_data = any(prev_data.get(k) for k in ("youtube", "instagram", "facebook", "tiktok", "daily")) \
                       or prev_data.get("downloads")
        except Exception:
            had_data = False
        if had_data:
            print("\n✗ Refusing to overwrite dashboard/data.js — this run pulled 0 from every")
            print("  source, which almost always means your API keys aren't loaded. Check that")
            print("  .env and the .p8 file are present in the project root, then re-run.")
            print("  (Your existing dashboard data was left untouched.)\n")
            sys.exit(1)
    # ---------------------------------------------------------------------------
